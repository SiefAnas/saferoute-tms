...
|---|---|---|---|
| **Driver week schedule** | Driver app "Week" tab opens Coming Soon; `/driver/week` shows a Coming Soon card. Today's schedule (with overrides) is on the Today tab. | `GET /schedule/week?from=YYYY-MM-DD`: same item shape as `/schedule/today`, one entry per weekday, with each day's `assignment_schedule_overrides`, parent skips and no-shows applied. Pure SQL over existing tables. | S |
