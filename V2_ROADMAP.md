...
|---|---|---|---|
| **Driver week schedule** | Driver app "Week" tab opens Coming Soon; `/driver/week` shows a Coming Soon card. Today's schedule (with overrides) is on the Today tab. **UI still Coming soon.** | **Backend done** (branch `v2-week-schedule`): `GET /schedule/week?start=YYYY-MM-DD`, 7 days, morning/afternoon runs with the `/schedule/today` item shape, overrides, parent skips and no-shows applied, driver scope (see `API_CONTRACT.md`). Next: web Week tab + mobile Week tab. | S (UI) |
